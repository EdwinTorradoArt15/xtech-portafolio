    ...
    <div className="max-w-5xl p-6 mx-auto mt-10 md:-mt-40">
      <div className="justify-between md:flex">
        <div>
          ...
          />
          <Reveal>
            <p className="mt-5 text-dark max-w-[250px]">
          ...
          <div key={id}>
            ...
            {links.map(({ id, name, link }) => (
              <Link
                key={id}
                href={link}
                className="block mt-4 text-dark hover:text-secondary transition-all duration-300 ease-in-out"
              >
                <Reveal>{name}</Reveal>
              </Link>
            ))}
      ...
      <div className="items-center justify-between md:flex">
        <div className="my-3">
          <Reveal>2023 XTECH. Todos los derechos reservados.</Reveal>
        </div>
